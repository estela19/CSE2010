...
void Search(BTreeNode* Node, int Key) {
    if (Node->Leaf == TRUE) {
        ...
        if (isExist == TRUE) printf("Key %d exist in tree\n\n", Key);
        else printf("Key %d not exist in tree\n\n", Key);
        return;
    }
    ...
    for (int i = 0; i < Node->KeyIndex; i++) {
        ...
        if (Key < Node->Keys[0]) {
            ...
            return Search(Node->Childs[i + 1], Key);
        }
        else if (Node->Keys[Node->KeyIndex] < Key) {
            return Search(Node->Childs[Node->KeyIndex + 1], Key);
        }
    }
}
...
void SplitChild(BTreeNode* Parent, BTreeNode* Child, int Index) {
    ...
    for (int i = Parent->KeyIndex; i > Index - 1; --i) {
        Parent->Childs[i + i] = Parent->Childs[i];
    }
...
void RemoveFromLeaf(BTreeNode* Node, int Index) {
    // remove the index the key form node
...
void Merge(BTreeNode* Node, int Index) {
    ...
    // moving the child pointers after index + 1 in the Node ond step before
