...
DATATYPE Find(BTTree* Tree, DATATYPE* Data)
{
    DATATYPE tmp = Search(Tree, Data);
    if (tmp <= 0) return *Data;

    return Find(Tree, &tmp);
} 

int Union(BTTree* Tree1, BTTree* Tree2)
{
    DATATYPE* t1 = &(Tree1->Root->Data);
    DATATYPE* t2 = &(Tree2->Root->Data);
    
    if (Search(Tree1, t1) < Search(Tree2, t2))
    {
        DATATYPE tmp;
        PosHead(Tree2, &tmp);
        InsertBST(&(Tree1->Root), tmp);

        while (PosNext(Tree2, &tmp)) {
            InsertBST(&(Tree1->Root), tmp);
        }
    ...
    else
    {
        DATATYPE tmp;
        PosHead(Tree1, &tmp);
        InsertBST(&(Tree2->Root), tmp);

        while (PosNext(Tree1, &tmp)) {
            InsertBST(&(Tree2->Root), tmp);
        }
